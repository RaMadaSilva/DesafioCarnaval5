...
{
    private static readonly Dictionary<double, string> primeiraOrdem = new Dictionary<double, string>{
    ...
    private static readonly Dictionary<double, string> segundaOrdem = new Dictionary<double, string>{
        {20, "vinte"}, {30, "trinta"},{40, "quarenta"}, {50, "cinquenta"}, {60, "secenta"}, 
        {70, "setenta"}, {80, "oitenta"}, {90, "noventa"}};
    private static readonly Dictionary<double, string> terceiraOrdem = new Dictionary<double, string>{
    ...
    private static readonly Dictionary<double, string> quartaOrdem = new Dictionary<double, string>{
                ...
                if (valorCentena!=0 && restoCentena % 10 == 0)
                {
                    foreach (var item in terceiraOrdem.Keys)
                    {
                        if (valorCentena * 100 == item)
                            return sb.Append($"{quartaOrdem[valorMilhar*1000]} e {terceiraOrdem[item]}").ToString();
                    }
                }
                else
                {
